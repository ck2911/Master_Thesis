...
The canonical baseline aggregation is the market-magnitude weighted quarterly sum of the ABGMR `target` factor:
...
The pipeline also exports diagnostic transformations for every factor:
...
The `timing_factor`, `fg_factor`, and `qe_factor` series are preserved for source audit, decomposition, and transmission diagnostics. They are not intended to enter the baseline LP-IV simultaneously.
