...
This is the canonical reduced-form transmission engine. It estimates monthly local projections from high-frequency ECB surprise shocks to real monthly response variables and accepted monthly proxies. Coefficients are reported as responses to a one-standard-deviation ECB surprise; the raw per-unit coefficients are retained in the CSVs for auditability.
...
All IRF tables include HAC/Newey-West confidence bands with horizon-specific bandwidth `h + 1`. Full-sample target-shock IRFs also include feasible circular-block residual percentile bootstrap intervals. The refined uncertainty layer adds moving-block and wild bootstrap intervals with joint horizon residual resampling for cumulative bands. Cumulative responses are used for persistence language, not exact structural multipliers. Additional uncertainty artifacts are written to `results/final/uncertainty/`.
...
Allowed language: ECB monetary-policy surprises generate dynamic reduced-form response patterns in monthly financial, housing-finance, real-activity, and negotiated-wage-pressure variables.

Disallowed language: these LPs identify structural QE treatment effects, exact housing-price effects, compensation-per-employee treatment effects, welfare effects, or redistribution magnitudes.
