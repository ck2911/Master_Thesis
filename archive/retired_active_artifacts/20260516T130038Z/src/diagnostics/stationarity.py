...
def _safe_adf(series: pd.Series, regression: str = "c") -> dict[str, float | int | str]:
    clean = series.dropna()
    if clean.shape[0] < 12 or clean.nunique() < 3:
        return {"stat": np.nan, "pvalue": np.nan, "used_lag": np.nan, "nobs": clean.shape[0], "error": "too few observations or near-constant"}
    try:
        stat, pvalue, usedlag, nobs, *_ = adfuller(clean, regression=regression, autolag="AIC")
        return {"stat": float(stat), "pvalue": float(pvalue), "used_lag": int(usedlag), "nobs": int(nobs), "error": ""}
    except Exception as exc:
        return {"stat": np.nan, "pvalue": np.nan, "used_lag": np.nan, "nobs": clean.shape[0], "error": str(exc)}


def _safe_kpss(series: pd.Series, regression: str = "c") -> dict[str, float | int | str]:
    clean = series.dropna()
    if clean.shape[0] < 12 or clean.nunique() < 3:
        return {"stat": np.nan, "pvalue": np.nan, "used_lag": np.nan, "error": "too few observations or near-constant"}
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            stat, pvalue, usedlag, _ = kpss(clean, regression=regression, nlags="auto")
        return {"stat": float(stat), "pvalue": float(pvalue), "used_lag": int(usedlag), "error": ""}
    except Exception as exc:
        return {"stat": np.nan, "pvalue": np.nan, "used_lag": np.nan, "error": str(exc)}


def _safe_pp(series: pd.Series, trend: str = "c") -> dict[str, float | int | str]:
    clean = series.dropna()
    if clean.shape[0] < 12 or clean.nunique() < 3:
        return {"stat": np.nan, "pvalue": np.nan, "used_lag": np.nan, "error": "too few observations or near-constant"}
    if PhillipsPerron is None:
        return {"stat": np.nan, "pvalue": np.nan, "used_lag": np.nan, "error": "arch is not installed"}
    try:
        test = PhillipsPerron(clean, trend=trend)
        return {"stat": float(test.stat), "pvalue": float(test.pvalue), "used_lag": int(test.lags), "error": ""}
    except Exception as exc:
        return {"stat": np.nan, "pvalue": np.nan, "used_lag": np.nan, "error": str(exc)}
...
def classify_integration(row: pd.Series, alpha: float = 0.05) -> str:
    ...
    if pd.notna(adf_level) and pd.notna(kpss_level) and adf_level < alpha and kpss_level > alpha:
        return "I(0)"
    if all(pd.notna(x) for x in [adf_level, kpss_level, adf_diff, kpss_diff]):
        if adf_level > alpha and kpss_level < alpha and adf_diff < alpha and kpss_diff > alpha:
            return "I(1)"
    if pd.notna(adf_trend) and pd.notna(kpss_trend) and adf_trend < alpha and kpss_trend > alpha:
        return "trend-stationary"
    if pd.notna(adf_diff) and pd.notna(kpss_diff) and adf_diff < alpha and kpss_diff > alpha:
        return "difference-stationary/ambiguous"
    if pd.notna(adf_diff) and adf_diff < alpha:
...
def stationarity_table(data: pd.DataFrame, alpha: float = 0.05) -> pd.DataFrame:
    ...
    for variable in data.columns:
        ...
        row = {
            ...
            "notes": "; ".join(sorted({str(x) for x in [adf_c_level["error"], kpss_c_level["error"], pp_c_level["error"]] if x})),
        }
