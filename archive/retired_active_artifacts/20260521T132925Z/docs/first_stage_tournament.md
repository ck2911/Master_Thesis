...
This tournament replaces the old reflex of instrumenting quarterly ECB assets. It evaluates whether the high-frequency ECB surprise itself has monthly first-stage relevance for candidate bridge variables. The table is a screening device for causal architecture, not a response result.
...
ECB asset stocks are retained as liquidity responses or secondary robustness variables. They are not automatically selected as the central treatment merely because they were used in the previous quarterly LP-IV system.
...
| requested_candidate | status | governance_decision |
| --- | --- | --- |
...
## Causal-Governance Implication

The treatment variable must be selected from this table before any monthly LP-IV response interpretation. If no bridge variable remains strong after stability and event-sensitivity checks, the thesis must pivot to disciplined non-IV monthly local projections and treat the high-frequency surprises as reduced-form shocks rather than forcing a structural IV claim.
