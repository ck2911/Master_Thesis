...
Legacy timing-factor first-stage evidence for `timing_factor_quarterly_sum`:
...
Current LP-IV baseline: `target_factor_market_magnitude_weighted_quarterly_sum` against `d_ecb_assets_ea_qavg`. The timing-factor rows above are retained as historical screening evidence only.
...
For the legacy timing instrument, crisis regimes account for 49.5% of total absolute quarterly surprise mass. The top decile of absolute timing surprises accounts for 42.1% of total absolute mass. Outlier quarters with |z| >= 3: `2006Q2,2008Q2,2009Q3,2011Q3`.
...
- Robustness instruments: weighted shocks, QE-only variants, signed cumulative variants, quarterly means, and monthly bridge variants.
...
Next allowed stage: LP-IV infrastructure validation with first-stage, rolling, regime, and horizon diagnostics. Final SVECM estimation remains closed.
